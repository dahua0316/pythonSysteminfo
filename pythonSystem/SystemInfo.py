...
def GetGpuInfo(gpunum: int = 0) -> dict:
    '''
    获取GPU信息 名字 显存大小 已用显存大小 剩余显存大小
    :param gpunum:
      gpunum 指定显卡型号 default is 0
    :return:
      Dict
    '''
    pynvml.nvmlInit()
    handle = pynvml.nvmlDeviceGetHandleByIndex(gpunum)
    meminfo = pynvml.nvmlDeviceGetMemoryInfo(handle)
    gpuName = os.popen("nvidia-smi -L", "r")
    gpumemory: dict = {
        'gpuName': str(gpuName.read()),
        'gpuMemtotal': ToSizeInt(meminfo.total, 'MB'),
        'gpuMemused': ToSizeInt(meminfo.used, 'MB'),
        'gpuMemfree': ToSizeInt(meminfo.free, 'MB')
    }

    return gpumemory


def GetProcess(processName: str = None) -> dict:
    '''
    获取某个进程cpu 占用率 总内存利用率
    :param Process: 进程名称不区分大小写
    :return: 进程cpu 占用率 总内存利用率
    '''
    if processName == None:
        raise ValueError("pocessName 不允许为空")
    process = []
    for pros in psutil.process_iter():
        process.append(pros.name().lower())
        if pros.name().lower() == processName.lower():
            processInfo: dict = {
                "cpuOcupancy": winOrUnix(pros.pid),
                "proPid": pros.pid,
                "proName": pros.name(),
                "memRss": ToSizeInt(pros.memory_info().rss, 'MB'),
                "memUseratio": pros.memory_percent()
            }
            return processInfo

    if processName not in process:
        raise ValueError(processName + "没有运行")


def winOrUnix(proPid:int)->int:

    if UNIX:
        return psutil.Process(proPid).cpu_percent(interval=1)
    else:
        return psutil.Process(proPid).cpu_percent(interval=1)/psutil.cpu_count()
